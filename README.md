...
- **Studio page** (`/w/<token>/studio/<roster_id>`) has two modes, toggled top-right, and remembers which one you were last in per roster (new rosters always start in Roster mode):
  - **Roster mode** — build the roster itself: add/remove players and alts, edit name/experience/preferred/secondary/**unwilling** positions. No solver here (nothing auto-runs while you're just entering data), and no scenario-only levers (availability, position override, player link) — a roster has no concept of those. A given position can only be claimed by one of preferred/secondary/unwilling at a time — checking it in one column clears it from the other two automatically, like a radio group per position (with "none checked" still valid — that's just implicitly OOP). Any pre-existing ambiguous data (an older save, or a CSV whose columns disagreed) is resolved the same way, preferred > secondary > unwilling, the moment it's loaded. Name and Exp columns are sortable (click to reorder, click again to reverse — a one-time reorder, not continuously reapplied, so a row you're editing doesn't jump around).
  - **Scenario mode** — the what-if workspace. The solver runs here (auto, debounced, after every edit; or manually via "Solve now"), and the scenario-only levers reappear. Players that came from the roster keep their name locked and can't be deleted — toggling **available** off is how you exclude one, with the same effect as deleting them; every scenario starts with everyone available unless you deselect some, and a branch inherits its parent's deselections. Players added while already in Scenario mode are fully yours: editable, deletable, and they travel with the scenario if you save it, never touching the roster itself. A result slot can be flagged **unwilling** (red badge) instead of primary/secondary/OOP — only reachable when a position override forces a player onto a position they marked unwilling to play, which is otherwise a hard constraint the solver would never violate on its own.
...
- **Save** (the split button) is context-sensitive:
  - In Roster mode: **Save roster** (overwrites the saved roster, including a title edit; confirms if there are unsaved changes) and **Save roster as…** (forks into a new, independent roster).

...
- **Studio page** (`/w/<token>/studio/<roster_id>`) has two modes, toggled top-right, and remembers which one you were last in per roster (new rosters always start in Roster mode):
  - **Roster mode** — build the roster itself: add/remove players and alts, edit name/availability/experience/preferred/secondary/**unwilling** positions. No solver here (nothing auto-runs while you're just entering data), and no scenario-only levers (position override, player link) — a roster has no concept of those.
  - **Scenario mode** — the what-if workspace. The solver runs here (auto, debounced, after every edit; or manually via "Solve now"), and the two scenario-only levers reappear. Players that came from the roster keep their name locked and can't be deleted — toggling **available** off is how you exclude one, with the same effect as deleting them. Players added while already in Scenario mode are fully yours: editable, deletable, and they travel with the scenario if you save it, never touching the roster itself.
